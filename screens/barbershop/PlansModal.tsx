      ...
      <div className="bg-brand-dark w-full max-w-4xl rounded-lg shadow-xl p-6 relative max-h-[90vh] flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto pr-2 grid grid-cols-1 md:grid-cols-3 gap-6">
          {PLANS.map(plan => {
            ...
            return (
              <div key={plan.name} className={`bg-brand-secondary p-6 rounded-lg flex flex-col border-2 ${plan.id === 'PRO' ? 'border-brand-primary' : 'border-gray-700'} transition`}>
                <h3 className="font-bold text-2xl text-amber-300">{plan.name}</h3>
                <div className="my-4">
                    <p className="font-bold text-3xl text-white">R$ {monthlyEquivalent.toFixed(2)}<span className="text-lg text-gray-400">/mês</span></p>
                    {isAnnual && <p className="text-xs text-gray-400">Cobrado R$ {price.toFixed(2)} anualmente</p>}
                </div>
                ...
                <ul className="text-sm text-gray-300 space-y-2 flex-grow">
                  ...
                  <li className={`flex items-start gap-2 ${showOnlinePayments ? '' : 'text-gray-500 line-through'}`}>
                    ...
                    <span>Pagamentos Online {plan.id === 'BASIC' && isAnnual ? '(Incluso no Anual!)' : ''}</span>
                  </li>
        ...
        <div className="mt-8 text-center">
            <p className="text-gray-400 mb-4">Para contratar ou alterar seu plano, por favor, entre em contato com nosso suporte.</p>
            <Button onClick={onClose} variant="secondary" className="max-w-xs mx-auto">Fechar</Button>
        </div>
      </div>
    </div>
  );
