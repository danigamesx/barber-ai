        ...
        <header className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${isScrolled ? 'bg-brand-secondary/80 backdrop-blur-lg' : 'bg-transparent'}`}>
            <nav className="flex items-center justify-between p-4 lg:px-8 max-w-7xl mx-auto" aria-label="Global">
                ...
                <div className="hidden lg:flex lg:flex-1 lg:justify-end lg:items-center lg:gap-x-4">
                    <Button onClick={() => onEnter('client')} variant="secondary" className="py-2 px-4 text-sm w-auto">Sou Cliente</Button>
                    <Button onClick={() => onEnter('barbershop')} variant="primary" className="py-2 px-4 text-sm w-auto">Teste Grátis</Button>
                </div>

                {/* BOTÕES MOBILE - canto direito */}
                <div className="flex lg:hidden items-center gap-x-2">
                    <Button onClick={() => onEnter('client')} variant="secondary" className="py-2 px-3 text-sm w-auto whitespace-nowrap">
                        Sou Cliente
                    </Button>
                ...
                <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-brand-dark px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-white/10">
                    ...
                    <div className="mt-6 flow-root">
                        <div className="-my-6 divide-y divide-gray-500/25">
                            ...
                            <div className="py-6">
                                <Button onClick={() => onEnter('barbershop')} variant="primary" className="w-full">Teste Grátis por 30 Dias</Button>
                            </div>
                        </div>
                    </div>
                </div>
